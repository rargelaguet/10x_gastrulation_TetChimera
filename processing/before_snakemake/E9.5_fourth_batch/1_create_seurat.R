...
# Define I/O
io$inputdir <- paste0(io$basedir,"/original/no_tomato/fourth_batch")
io$outputdir <- paste0(io$basedir,"/processed/fourth_batch")
...
gene.loc <- sprintf("%s/%s_features.tsv.gz",io$inputdir,opts$batches[[1]])
...
for (i in opts$batches) {
  ...
  # matrix.loc <- sprintf("%s/%s_soupX_adjusted_matrix.mtx.gz",io$inputdir,i)
