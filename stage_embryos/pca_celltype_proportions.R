...
# opts$classes <- c("E7.5_WT_tdTomato+", "E7.5_WT_tdTomato-", "E7.5_TET_TKO", "E8.5_WT_tdTomato+", "E8.5_WT_tdTomato-", "E8.5_TET_TKO")
opts$classes <- c("E7.5_WT","E7.5_TET_TKO","E8.5_WT", "E8.5_TET_TKO")
...
cell_metadata.dt <- fread(io$metadata) %>% 
  ...
  .[,class:=sprintf("%s_%s",stage,class2)] %>%
...
celltype_proportions_query.dt <- cell_metadata.dt %>% copy %>%
  ...
  .[,.(celltype_proportion=.N/unique(N)),by=c("alias","class","celltype")] %>%
  .[,dataset:="Query"]
...
celltype_proportions_atlas.dt <- fread(paste0(io$atlas.basedir,"/results/celltype_proportions/celltype_proportions_noExE.txt.gz")) %>%
...
pdf(file.path(io$outdir,"boxplots_embryo_stage_all_classes.pdf"), width=6.5, height=6.5)
print(p)
dev.off()
